...
const BattleText = textJSON as {[id: string]: {[templateName: string]: string}};
...
class TextParser {
	...
	static parseBattleLine(line: string): {args: Args, kwArgs: KWArgs} {
		...
		return TextParser.upgradeArgs({args, kwArgs});
	}
	...
	extractMessage(buf: string) {
		...
		for (const line of buf.split('\n')) {
			const {args, kwArgs} = TextParser.parseBattleLine(line);
	...
	lineSection(args: Args, kwArgs: KWArgs) {
		...
		case '-activate': {
			const id = TextParser.effectId(args[2]);
